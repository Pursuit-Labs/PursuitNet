...
class TestModelCreation(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.custom_fc1_weights = np.random.normal(0.0, np.sqrt(2. / (28*28)), (28*28, 512)).astype(np.float32)
        self.custom_fc1_bias = np.zeros(512, dtype=np.float32)
        self.custom_fc2_weights = np.random.normal(0.0, np.sqrt(2. / 512), (512, 10)).astype(np.float32)
        ...
        class PyTorchNet(torch_nn.Module):
            ...
            def init_weights(self, fc1_weights, fc1_bias, fc2_weights, fc2_bias):
                with torch.no_grad():
                    self.fc1.weight = torch.nn.Parameter(torch.tensor(fc1_weights.T, dtype=torch.float32))
                    self.fc1.bias = torch.nn.Parameter(torch.tensor(fc1_bias, dtype=torch.float32))
                    self.fc2.weight = torch.nn.Parameter(torch.tensor(fc2_weights.T, dtype=torch.float32))
        ...
        self.pytorch_model = PyTorchNet(self.custom_fc1_weights, self.custom_fc1_bias, self.custom_fc2_weights, self.custom_fc2_bias)
        self.custom_model = CustomNet(self.custom_fc1_weights, self.custom_fc1_bias, self.custom_fc2_weights, self.custom_fc2_bias)
        ...
        self.custom_input = self.input_data
    ...
    def test_forward_output(self):
        ...
        # Compare outputs
        np.testing.assert_allclose(custom_output, pytorch_output, rtol=1e-5, atol=1e-8)
    ...
    def test_intermediate_activation(self):
        ...
        x = F.relu(custom_intermediate_activations[-1])
        ...
        for idx, custom_activation in enumerate(custom_intermediate_activations):
            ...
            np.testing.assert_allclose(custom_activation, pytorch_activation, rtol=1e-5, atol=1e-8)
